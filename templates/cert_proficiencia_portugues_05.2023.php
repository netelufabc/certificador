...
$fpdf->Image($assinatura, 35, 142, 95);
...
$fpdf->Image($assinatura, 105, 140, 115);
...
$fpdf->Image($assinatura, 195, 137, 110);
...
//siape
$fpdf->SetXY(210, 183);
$fpdf->MultiCell(0, 7, utf8_decode("Siape 1247586"), 0, 'C');
...
//conceitos das avaliações; não entra aqui se não tiver notas
if (count($notas) > 0) {
    $i = 1;
    $setY = 168;
    foreach ($notas as $nota) {
        $fpdf->SetXY(22, $setY);
        $fpdf->SetFont('arial', '', 10);
        $fpdf->MultiCell(253, 7, utf8_decode("Conceito da avaliação " . $i . ": " . $nota['nota']), 0, 'J');
        $i++;
        $setY += 5;
    }
}
