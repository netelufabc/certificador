...
//        $res = $this->db->query("SELECT * FROM 'alunocurso' WHERE 'id' = $id AND 'cod_validacao' = '$cod_validacao';")->result();
    ...
    function getCursosDoAluno($emailAluno) {
        $sql = "SELECT a.`id_aluno`,a.`nome`,a.`email`, c.`id_curso`,c.`nomeCurso`,c.`dataIni`,c.`dataFim`, ac.`id_alunocurso`, ac.`id_aluno`, ac.`id_curso`, ac.`cod_validacao`, ac.`status`
    ...
    function getCursosDoAlunoPorNome($nomeAluno) {
        $sql="SELECT a.id_aluno, a.nome, a.email,c.id_curso, c.nomeCurso, ac.id_alunocurso, ac.id_certificado, ac.cod_validacao, ac.status
...
//            ORDER BY c.`created` DESC;";
    ...
    function getAlunosDoCurso($id_curso) {
        $sql = "SELECT ac.`id_alunocurso`, ac.`id_aluno`,ac.`id_curso`,ac.`conceito`,ac.`cod_validacao`, ac.`status` ,a.`nome`, a.`email`,a.`statusEmail`, c.`nomeCurso`,c.`edital`,c.`dataIni`, c.`dataFim`
            FROM cert_alunocurso ac, cert_alunos a, cert_cursos c
    ...
    function get_validacao($id_alunocurso, $cod_validacao) {
        ...
        $sql = "SELECT a.`nome`, c.`nomeCurso`, c.`cargaHoraria`, c.`dataIni`, c.`dataFim`,  c.`cargaHoraria`, ac.`conceito`, ac.`faltas_em_horas`, c.`nivel`
            FROM cert_alunos a, cert_cursos c, cert_alunocurso ac
    ...
    public function delAlunoDoCurso($dados) {
        if ($dados != NULL) {
            $this->db->delete('cert_alunocurso', $dados);
            return TRUE;
        }
    }
